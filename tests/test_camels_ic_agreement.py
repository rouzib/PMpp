...
def _build_confs(pair, mesh_shape):
    ...
    conf_pmpp = Configuration(
        ptcl_spacing, (grid_size, ) * 3, mesh_shape=mesh_shape,
        multigpu=MultiGPUConfiguration(compute_mesh=compute_mesh, mode="mesh_halo"),
        max_ptcl_per_slice=max_ptcl_per_slice, max_share_ptcl=MAX_SHARE_PTCL, max_halo_share_ptcl=MAX_SHARE_PTCL,
        max_share_gather_ptcl=MAX_SHARE_PTCL, a_start=meta.a_start, a_stop=1.0, a_nbody_maxstep=A_NBODY_MAXSTEP,
    )
    conf_pmwd = ConfigurationPMWD(
        ptcl_spacing=ptcl_spacing, ptcl_grid_shape=(grid_size, ) * 3, mesh_shape=mesh_shape, a_start=meta.a_start,
        a_stop=1.0, a_nbody_maxstep=A_NBODY_MAXSTEP,
    )
...
def _build_cosmologies(pair, conf_pmpp, conf_pmwd):
    ...
    cosmo_pmpp = Cosmology.from_sigma8(
        conf_pmpp, sigma8=meta.sigma8, n_s=meta.n_s, Omega_m=meta.omega_m, Omega_b=meta.omega_b, h=meta.h,
    )
    cosmo_pmpp = boltzmann_pmpp(cosmo_pmpp, conf_pmpp)

    cosmo_pmwd = CosmologyPMWD.from_sigma8(
        conf_pmwd, sigma8=meta.sigma8, n_s=meta.n_s, Omega_m=meta.omega_m, Omega_b=meta.omega_b, h=meta.h,
    )
...
def _assert_full_forward_agreement(label, dens_pmpp, dens_pmwd):
    ...
    cosine = float(
        np.dot(dens_pmpp.ravel(), dens_pmwd.ravel()) /
        (np.linalg.norm(dens_pmpp.ravel()) * np.linalg.norm(dens_pmwd.ravel()))
    )
    assert np.isclose(dens_pmpp.mean(), 1.0, atol=5e-6), f"{label}: PM++ mean drifted from mass conservation"
    assert np.isclose(dens_pmwd.mean(), 1.0, atol=5e-6), f"{label}: PMWD mean drifted from mass conservation"
...
if pytest is not None:
    test_camels_ic_path_matches_pmwd = pytest.mark.parametrize(
        "mesh_shape", MESH_SHAPES, ids=[f"mesh{mesh_shape}" for mesh_shape in MESH_SHAPES],
