...
def test_cuda_ffi_wrappers_preserve_all_shapes_dtypes_targets_and_attributes(monkeypatch):
    ...
    for dtype, words, suffix in ((jnp.float32, 8, ""), (jnp.float64, 14, "_f64")):
        ...
        assert [value.shape for value in bidir] == [(3, words), (3, words), (), (), (4, ), (3, ), (3, ), ()]
        ...
        merged_bidir = cuda.route_merge_bidir_cuda(
            pmid, disp, vel, jnp.arange(4, dtype=jnp.uint32), jnp.arange(4, dtype=jnp.int32), jnp.int32(3), records,
            jnp.int32(2), records, jnp.int32(1), mesh_shape=(8, 2, 1), capacity=4,
        )
        assert [value.shape for value in merged_bidir] == [(4, 3), (4, 3), (4, 3), (4, ), (4, ), (4, ), (4, ), ()]
...
def test_cuda_float_abi_rejects_mixed_unsupported_and_unregistered_payloads(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="no float64 bidirectional ABI"):
        cuda.route_merge_bidir_cuda(
            pmid, disp, vel, jnp.zeros(4, jnp.uint32), jnp.zeros(4, jnp.int32), jnp.int32(0),
            jnp.zeros((4, 14), jnp.uint32), jnp.int32(0), jnp.zeros((4, 14), jnp.uint32), jnp.int32(0),
            mesh_shape=(8, 1, 1), capacity=4,
        )


def _reset_registration(monkeypatch):
    for name in ("_REGISTERED", "_BIDIR_REGISTERED", "_FLOAT64_REGISTERED", "_FLOAT64_BIDIR_REGISTERED"):
...
def test_cuda_target_registration_qualifies_complete_typed_library(monkeypatch):
    ...
    targets = cuda._CURRENT_TARGETS + cuda._BIDIR_TARGETS + cuda._FLOAT64_TARGETS + cuda._FLOAT64_BIDIR_TARGETS
...
def test_cuda_environment_manifest_and_library_discovery_are_failure_safe(tmp_path, monkeypatch):
    ...
    good_manifest.write_text('{"record_format_version": 2}', encoding="utf-8")
    monkeypatch.setenv("PMPP_CUDA_ROUTING_MANIFEST", str(good_manifest))
    assert cuda._load_build_manifest() == {"record_format_version": 2}
