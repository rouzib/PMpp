...
def _sinusoid_positions(res, box_size, amplitude):
    ...
    pos[:, 0] = (pos[:, 0] +
                 np.float32(amplitude) * np.sin(np.float32(2 * np.pi / box_size) * pos[:, 0])) % np.float32(box_size)
    return pos
...
def _x_force_modes(mesh_shape):
    ...
    box_size = 1000.0
    conf = Configuration(
        box_size / res, (res, res, res), mesh_shape=mesh_shape, a_start=1 / 128, a_stop=1.0, a_nbody_maxstep=1 / 20,
    )
    pos = _sinusoid_positions(res, box_size, amplitude=0.5)
    ptcl = Particles.from_ordered_pos(conf, jnp.asarray(pos), vel=jnp.zeros_like(jnp.asarray(pos)), )
