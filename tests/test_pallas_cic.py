...
pytestmark = pytest.mark.skipif(
    not pallas_cic_supported(jnp.float32),
    reason="Pallas CIC tests require a qualified float32 GPU JAX installation",
)
...
def _inputs(conf):
    ...
    pmid = jnp.array(
        [[0, 0, 0], [1, 2, 3], [3, 0, 2], [2, 3, 1]],
        dtype=conf.pmid_dtype,
    )
    disp = jax.random.uniform(
        key, (4, 3), minval=-0.35, maxval=0.35, dtype=conf.float_dtype
    )
...
def test_pallas_cic_explicit_mesh_cell_size():
    ...
    actual_mesh = _scatter(
        pmid, disp, conf, jnp.zeros_like(mesh), val, offset, 0.5
    )
    expected_mesh = _scatter(
        pmid, disp, reference, jnp.zeros_like(mesh), val, offset, 0.5
    )
...
def test_pallas_cic_masks_final_particle_tile():
    ...
    disp = jax.random.uniform(
        key, (5, 3), minval=-0.45, maxval=0.45, dtype=conf.float_dtype
    )
    mesh = jax.random.normal(key, (4, 4, 4, 2), dtype=conf.float_dtype)
...
def test_pallas_cic_validity_mask_and_three_channels():
    ...
    actual_scatter = _scatter(
        pmid, disp, conf, jnp.zeros((4, 4, 4, 3), dtype=conf.float_dtype),
        values, 0, None, valid,
    )
    expected_scatter = _scatter(
        pmid, disp, reference, jnp.zeros((4, 4, 4, 3), dtype=conf.float_dtype),
        jnp.where(valid[:, None], values, 0), 0, None,
    )
    np.testing.assert_allclose(actual_scatter, expected_scatter, rtol=3e-5, atol=3e-5)

    def scatter_loss(d, v):
        out = _scatter(
            pmid, d, conf, jnp.zeros((4, 4, 4, 3), dtype=conf.float_dtype),
            v, 0, None, valid,
        )
    ...
    def scatter_reference_loss(d, v):
        out = _scatter(
            pmid, d, reference, jnp.zeros((4, 4, 4, 3), dtype=conf.float_dtype),
            jnp.where(valid[:, None], v, 0), 0, None,
        )
        return jnp.sum(out ** 2)
