...
class LPT3(object):
    ...
    def _getWaveVectorNorms(self):
        ...
        return (self.wave_numbers**2 + self.wave_numbers[:, None]**2 +
                self.wave_numbers[:, None, None]**2)[:, :, :self.num_modes_last_d]
    ...
    def getLinearDelta(self, k, p, sphere_mode=True):
        ...
        noise = np.fft.rfftn(
            np.random.randn(self.num_mesh_1d, self.num_mesh_1d, self.num_mesh_1d)
        ) / self.num_mesh_1d**1.5
        if sphere_mode:
