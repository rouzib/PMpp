...
def _init_confs():
    ...
    conf_pmpp = Configuration(
        ...
        max_ptcl_per_slice=int(num_ptcl**3 / 2 * 1.7), max_share_ptcl=2000, max_share_gather_ptcl=6000,
        to_save_z=[1, 2 / 3, 1 / 3, 0], a_start=1 / 60, a_nbody_maxstep=1 / 60, pallas_cic=False,
    ...
    conf_pmwd = ConfigurationPMWD(
        ...
        a_start=conf_pmpp.a_start, a_nbody_maxstep=conf_pmpp.a_nbody_maxstep, cosmo_dtype=jnp.float64,
...
def test_linear_modes_gradients_match_pmwd():
    ...
    assert np.allclose(
        np.asarray(jax.device_get(white_pmpp)), np.asarray(jax.device_get(white_pmwd)), atol=1e-8, rtol=1e-8,
    )
    ...
    def loss_pmwd(params):
        ...
        cosmo = CosmologyPMWD.from_sigma8(
            conf_pmwd, sigma8=sigma8_value, n_s=n_s, Omega_m=omega_m_value, Omega_b=omega_b, h=h,
        )
    ...
    def loss_pmpp(params):
        ...
        cosmo = CosmologyPMPP.from_sigma8(
            conf_pmpp, sigma8=sigma8_value, n_s=n_s, Omega_m=omega_m_value, Omega_b=omega_b, h=h,
        )
    ...
    fd_sigma8 = (float(loss_pmpp(params + jnp.array([1e-5, 0.0]))) -
                 float(loss_pmpp(params - jnp.array([1e-5, 0.0])))) / (2e-5)
    fd_omega_m = (
        float(loss_pmpp(params + jnp.array([0.0, 3e-4]))) - float(loss_pmpp(params - jnp.array([0.0, 3e-4])))
    ) / (6e-4)
