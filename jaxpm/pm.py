...
def make_neural_ode_fn(model, mesh_shape, density_correction=False, ):
    def neural_nbody_ode(state, a, cosmo, params):
        ...
        c = [cosmo.tree_flatten()[0][0], cosmo.tree_flatten()[0][4]]

        if density_correction:
            pot_k = pot_k * (1. + model.apply(params, kk, jnp.atleast_1d(a), c, delta_k))
        else:
            pot_k = pot_k * (1. + model.apply(params, kk, jnp.atleast_1d(a), c))
...
def make_neural_ode_fn_multiple(model, mesh_shape, density_correction=False, weights=None):
    indexWeights = True
    if weights is None:
        weights = jnp.ones(int(mesh_shape[0] / 2 + 1))
        indexWeights = False
    ...
    def neural_nbody_ode(state, a, cosmo, params):
        ...
        for i in range(len(params)):
            if density_correction:
                if indexWeights:
                    correction = correction + model.apply(params[i], kk, jnp.atleast_1d(a), c,
                                                          delta_k) * jnp.broadcast_to(
                        weights[i][jnp.newaxis, jnp.newaxis, :],
                        (mesh_shape[0], mesh_shape[0], int(mesh_shape[0] / 2) + 1))
                else:
                    correction = correction + model.apply(params[i], kk, jnp.atleast_1d(a), c,
                                                          delta_k) * jnp.broadcast_to(
                ...
                if indexWeights:
                    correction = correction + model.apply(params[i], kk, jnp.atleast_1d(a), c) * jnp.broadcast_to(
                        weights[i][jnp.newaxis, jnp.newaxis, :],
                        (mesh_shape[0], mesh_shape[0], int(mesh_shape[0] / 2) + 1))
                else:
                    correction = correction + model.apply(params[i], kk, jnp.atleast_1d(a), c) * jnp.broadcast_to(
        ...
        """jax.debug.print("Correction shape: {x}", x=correction.shape)
        print(f"Correction shape: {correction.shape}")"""

        pot_k = pot_k * correction
...
def make_neural_ode_fn_new_cosmo(model, mesh_shape, density_correction=False, ):
    def neural_nbody_ode_new_cosmo(state, a, cosmo, params):
        """
        state is a tuple (position, velocities)
        """
        cosmo = jc.Planck15(Omega_c=float(cosmo[0]), Omega_b=0.049, n_s=0.9624, h=0.671, sigma8=float(cosmo[1]))
    ...
    return neural_nbody_ode_new_cosmo
