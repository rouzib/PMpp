...
from .cuda import enabled_for_configuration as cuda_routing_enabled
...
def _capacity_check(count, capacity, message):
    ...
    _ = jax.lax.cond(
        count > capacity, lambda _: raise_error(message, x=count, y=capacity), lambda _: None, operand=None,
    )
...
def _gather_compacted(values, compact_idx, valid, fill_value):
    ...
    return jnp.where(valid.reshape(valid_shape), gathered, jnp.asarray(fill_value, dtype=values.dtype), )
...
def _compact_sorted_particles_no_acc_with_slots(keys, pmid, disp, vel, mask, capacity, key_fill, error_message):
...
def _linear_merge_plan_two(keys_a, valid_a, keys_b, valid_b, capacity, key_fill):
    ...
    source_code = source_code.at[pos_b].set(jnp.where(valid_b, jnp.int32(1), jnp.int32(0)), mode="drop")
...
def _sparse_stay_source_plan(valid_a, slots_left, valid_left, capacity, slots_right=None, valid_right=None, ):
    ...
    if slots_right is None:
        ...
        left_rank = left_index + jnp.searchsorted(right_filled, left_filled, side="left").astype(jnp.int32)
        right_rank = right_index + jnp.searchsorted(left_filled, right_filled, side="left").astype(jnp.int32)
        right_threshold = slots_right - right_rank
        right_threshold = jnp.where(valid_right, right_threshold, jnp.int32(capacity))
        hole_counts = hole_counts.at[right_threshold].add(valid_right.astype(jnp.int32), mode="drop")
    ...
    hole_counts = hole_counts.at[left_threshold].add(valid_left.astype(jnp.int32), mode="drop")
    ...
    stay_pos = jnp.where(stay_valid, jnp.clip(slots + skipped, 0, valid_a.shape[0] - 1), jnp.int32(-1), )
    return skipped, stay_pos, stay_valid, stay_count


def _sparse_route_plan_two(
    keys_a, valid_a, outgoing_keys, outgoing_valid, outgoing_slots, keys_b, valid_b, capacity, key_fill,
):
    ...
    source_code = source_code.at[pos_b].set(valid_b, mode="drop")
...
def _sparse_route_plan_one_incoming_two_outgoing(
    keys_a, valid_a, outgoing_left_keys, outgoing_left_valid, outgoing_left_slots, outgoing_right_keys,
    outgoing_right_valid, outgoing_right_slots, incoming_keys, incoming_valid, capacity, key_fill,
):
    ...
    slots = jnp.arange(capacity, dtype=jnp.int32)
    is_incoming = jnp.zeros((capacity, ), dtype=jnp.bool_).at[incoming_pos].set(incoming_valid, mode="drop")
...
def _merge_small_incoming_streams(incoming_b, incoming_c, key_fill):
    ...
    plan = _linear_merge_plan_two(keys_b, valid_b, keys_c, valid_c, capacity, key_fill)
...
def _sparse_route_merge_two(
    original, outgoing, outgoing_slots, incoming, capacity, key_fill, error_message, provenance=False,
...
def _sparse_route_merge_three(
    original, outgoing_left, outgoing_left_slots, outgoing_right, outgoing_right_slots, incoming_b, incoming_c,
    capacity, key_fill, error_message, provenance=False,
):
    ...
    combined, combined_tag, combined_idx = _merge_small_incoming_streams(incoming_b, incoming_c, key_fill)
    incoming_keys, *incoming_payload, incoming_valid = combined
    plan = _sparse_route_plan_one_incoming_two_outgoing(
        keys_a, valid_a, outgoing_left_keys, outgoing_left_valid, outgoing_left_slots, outgoing_right_keys,
        outgoing_right_valid, outgoing_right_slots, incoming_keys, incoming_valid, capacity, key_fill,
    )
    (is_incoming, a_idx, incoming_idx, stay_rank, out_valid, total, stay_pos, stay_valid, ) = plan
    _capacity_check(total, capacity, error_message)
    out_keys = _linear_take_two(keys_a, incoming_keys, is_incoming, a_idx, incoming_idx)
    fields = [
    ...
    merged = (out_keys, *fields, out_valid)
    if provenance:
        out_src_tag = jnp.where(is_incoming, combined_tag[incoming_idx], jnp.int32(0))
        out_src_tag = jnp.where(out_valid, out_src_tag, jnp.int32(3))
        out_src_idx = jnp.where(is_incoming, combined_idx[incoming_idx], stay_rank)
        out_src_idx = jnp.where(out_valid, out_src_idx, jnp.int32(-1))
        merged = (*merged, out_src_tag, out_src_idx)
    return merged, stay_pos, stay_valid
...
def _pack_left_halo_and_authoritative(
    left_keys, left_pmid, left_disp, left_vel, left_acc, left_valid, auth_keys, auth_pmid, auth_disp, auth_vel,
    auth_acc, auth_valid, max_ptcl_per_slice, halo_start, halo_end, global_nMesh, disp_size,
):
    ...
    _capacity_check(
        total, max_ptcl_per_slice, "[ERROR] Exceeded canonical particle storage capacity. "
...
def _pack_authoritative_only(auth_pmid, auth_disp, auth_vel, auth_acc, auth_valid, max_ptcl_per_slice, ):
    """Pack the canonical authoritative block without duplicating halo particles."""
    auth_count = jnp.sum(auth_valid)
    _capacity_check(
        auth_count, max_ptcl_per_slice, "[ERROR] Exceeded authoritative-only storage capacity. "
        "required_slots={x}, max_ptcl_per_slice={y}.",
    )
...
def _pack_authoritative_only_no_acc(auth_pmid, auth_disp, auth_vel, auth_valid, max_ptcl_per_slice, ):
...
def _canonical_authoritative_from_full(
    pmid, source_disp, carried_disp, vel, acc, unused_index, global_nMesh, disp_size, num_gpus, offsets, conf,
):
...
def _canonical_route_authoritative(
    keys, pmid, disp, vel, acc, valid, global_nMesh, max_values_to_share, left_perm, right_perm, num_gpus, disp_size,
    offsets, conf,
):
    ...
    *send_left_items, send_left_pos = _compact_sorted_particles_with_slots(
    ...
    if num_gpus == 2:
        ...
        merged, _, _ = _sparse_route_merge_two(
            original, send_left, send_left_pos, incoming_from_right, pmid.shape[0], key_fill,
    ...
    *send_right_items, send_right_pos = _compact_sorted_particles_with_slots(
    ...
    merged, _, _ = _sparse_route_merge_three(
        original, send_left, send_left_pos, send_right, send_right_pos, incoming_from_left, incoming_from_right,
        pmid.shape[0], key_fill, "[ERROR] Exceeded canonical authoritative capacity after migration. "
        "required_particles={x}, max_ptcl_per_slice={y}.",
    )
...
def _canonical_route_authoritative_no_acc_bidir_cuda(
    ...
    if _DEBUG_BIDIR_ROUTE:
        ...
        expected_classes = jnp.where(
            expected_stay, jnp.uint8(1),
            jnp.where(
                expected_left, jnp.uint8(2),
        ...
        reference, _ = _canonical_route_authoritative(
            keys, pmid, disp, vel, jnp.zeros_like(disp), valid, global_nMesh, max_values_to_share, left_perm,
            right_perm, num_gpus, disp_size, offsets, conf,
        )
        ...
        first_mismatch = jnp.min(jnp.where(row_mismatch, jnp.arange(pmid.shape[0], dtype=jnp.int32), pmid.shape[0]))
        safe_first = jnp.minimum(first_mismatch, pmid.shape[0] - 1)
        active_2d = active.reshape((-1, 1))
        disp_max_error = jnp.max(jnp.where(active_2d, jnp.abs(merged_disp - reference_disp), 0.0))
        vel_max_error = jnp.max(jnp.where(active_2d, jnp.abs(merged_vel - reference_vel), 0.0))
        ...
        _synchronized_nonzero_check(
            jnp.sum(row_mismatch), "[ERROR] Native bidirectional route disagrees with the canonical route. "
        ...
        jax.debug.print(
            ...
            "max|disp|={disp_max}; max|vel|={vel_max}", gpu=jax.lax.axis_index(AXIS_NAME),
            class_mismatch=jnp.sum(classes != expected_classes), native_left=send_left_count,
            native_right=send_right_count, native_stay=stay_count, expected_left=jnp.sum(expected_left),
            expected_right=jnp.sum(expected_right), expected_stay=jnp.sum(expected_stay),
            native_valid=jnp.sum(merged_valid), reference_valid=jnp.sum(reference_valid),
            row_mismatch=jnp.sum(row_mismatch), first=first_mismatch, native_key=merged_keys[safe_first],
...
def _canonical_route_authoritative_no_acc_cuda(
    ...
    if num_gpus == 2:
        ...
        merged_pmid, merged_disp, merged_vel, merged_valid = cuda_route_merge(
            pmid, disp, vel, stay_mask, incoming_from_right, incoming_from_right_count, mesh_shape=mesh_shape,
        ...
        send_right_records, send_right_count, _ = cuda_route_pack(
            pmid, disp, vel, valid, x_mod, global_nmesh=global_nMesh, mesh_shape=mesh_shape, owned_start=owned_start,
            owned_end=owned_end, slice_width=slice_width, direction=1, num_devices=num_gpus,
            capacity=max_values_to_share,
        )
        _synchronized_capacity_check(
            send_right_count, max_values_to_share, "[ERROR] Exceeded migration share capacity. "
            "particles_to_share={x}, max_share_ptcl={y}.",
        )
        incoming_from_left = jax.lax.ppermute(send_right_records, axis_name=AXIS_NAME, perm=right_perm)
        incoming_from_left_count = jax.lax.ppermute(send_right_count, axis_name=AXIS_NAME, perm=right_perm)
        ...
        merged_pmid, merged_disp, merged_vel, merged_valid = cuda_route_merge(
            pmid, disp, vel, stay_mask, incoming_from_left, incoming_from_left_count, mesh_shape=mesh_shape,
            capacity=pmid.shape[0],
        )
        incoming_from_right = jax.lax.ppermute(send_left_records, axis_name=AXIS_NAME, perm=left_perm)
        incoming_from_right_count = jax.lax.ppermute(send_left_count, axis_name=AXIS_NAME, perm=left_perm)
        ...
        _synchronized_capacity_check(
            jnp.sum(merged_valid_mask) + incoming_from_right_count, pmid.shape[0],
            "[ERROR] Exceeded canonical authoritative capacity after migration. "
            "required_particles={x}, max_ptcl_per_slice={y}.",
        )
        merged_pmid, merged_disp, merged_vel, merged_valid = cuda_route_merge(
            merged_pmid, merged_disp, merged_vel, merged_valid_mask, incoming_from_right, incoming_from_right_count,
            mesh_shape=mesh_shape, capacity=pmid.shape[0],
        )
...
def _canonical_route_authoritative_with_aux_bidir_cuda(
    ...
    stay_pos = jnp.compress(
        stay_mask, auth_slots, axis=0, size=auth_size, fill_value=jnp.asarray(-1, dtype=jnp.int32),
    )
    stay_valid = jnp.arange(auth_size, dtype=jnp.int32) < stay_count
    send_left_pos = jnp.compress(
        send_left_mask, auth_slots, axis=0, size=max_values_to_share, fill_value=jnp.asarray(-1, dtype=jnp.int32),
    )
    send_right_pos = jnp.compress(
        send_right_mask, auth_slots, axis=0, size=max_values_to_share, fill_value=jnp.asarray(-1, dtype=jnp.int32),
    )
    ...
    stay_acc = jnp.compress(stay_mask, acc, axis=0, size=auth_size, fill_value=jnp.asarray(0, dtype=acc.dtype), )
    send_left_acc = jnp.compress(
        send_left_mask, acc, axis=0, size=max_values_to_share, fill_value=jnp.asarray(0, dtype=acc.dtype),
    )
    send_right_acc = jnp.compress(
        send_right_mask, acc, axis=0, size=max_values_to_share, fill_value=jnp.asarray(0, dtype=acc.dtype),
    )
    incoming_from_left = jax.lax.ppermute(send_right_records, axis_name=AXIS_NAME, perm=right_perm)
    incoming_from_left_count = jax.lax.ppermute(send_right_count, axis_name=AXIS_NAME, perm=right_perm)
    ...
    merged_acc = jnp.where((merged_tag == 0).reshape((-1, 1)), stay_acc[safe_auth_idx],
                           jnp.where((merged_tag == 1).reshape((-1, 1)), incoming_from_left_acc[safe_share_idx],
                                     jnp.where((merged_tag == 2).reshape((-1, 1)),
                                               incoming_from_right_acc[safe_share_idx], zero_acc[:auth_size],
                                               ),
                                     ),
                           )
    merged_keys = pmid_to_idx(merged_pmid, conf)
    ...
    return (merged_keys, merged_pmid.astype(pmid.dtype), merged_disp, merged_vel, merged_acc, merged_valid, ), route_aux
...
def _canonical_route_authoritative_with_aux_cuda(
    ...
    if num_gpus == 2:
        merged_pmid, merged_disp, merged_vel, merged_valid, raw_tag, raw_idx = (
            cuda_route_merge(
                pmid, disp, vel, stay_mask, incoming_from_right, incoming_from_right_count, mesh_shape=mesh_shape,
                capacity=auth_size, auxiliary=True,
            )
        )
        merged_valid = merged_valid != 0
        merged_tag = jnp.where(merged_valid, jnp.where(raw_tag == 1, jnp.uint8(2), raw_tag), jnp.uint8(3))
        merged_idx = jnp.where(merged_valid, raw_idx, jnp.int32(-1))
        safe_auth_idx = jnp.clip(merged_idx, 0, max(auth_size, 1) - 1)
        safe_share_idx = jnp.clip(merged_idx, 0, max(max_values_to_share, 1) - 1)
        merged_acc = jnp.where((merged_tag == 0).reshape((-1, 1)), stay_acc[safe_auth_idx],
                               jnp.where((merged_tag == 2).reshape((-1, 1)), incoming_from_right_acc[safe_share_idx],
                                         zero_acc[:auth_size],
                                         ),
                               )
    ...
    _synchronized_capacity_check(
        jnp.sum(first_valid) + incoming_from_right_count, auth_size,
        "[ERROR] Exceeded canonical authoritative capacity after migration. "
        "required_particles={x}, max_ptcl_per_slice={y}.",
    )
    merged_pmid, merged_disp, merged_vel, merged_valid, raw_tag, raw_idx = (
        cuda_route_merge(
            first_pmid, first_disp, first_vel, first_valid, incoming_from_right, incoming_from_right_count,
            mesh_shape=mesh_shape, capacity=auth_size, auxiliary=True,
        )
    )
    merged_valid = merged_valid != 0
    ...
    merged_tag = jnp.where(
        merged_valid & (raw_tag == 0), prior_tag, jnp.where(merged_valid, jnp.uint8(2), jnp.uint8(3)),
    )
    merged_idx = jnp.where(merged_valid & (raw_tag == 0), prior_idx, jnp.where(merged_valid, raw_idx, jnp.int32(-1)), )
    safe_auth_idx = jnp.clip(merged_idx, 0, max(auth_size, 1) - 1)
    safe_share_idx = jnp.clip(merged_idx, 0, max(max_values_to_share, 1) - 1)
    merged_acc = jnp.where((merged_tag == 0).reshape((-1, 1)), stay_acc[safe_auth_idx],
                           jnp.where((merged_tag == 1).reshape((-1, 1)), incoming_from_left_acc[safe_share_idx],
...
def _canonical_route_authoritative_with_aux(
    ...
    if cuda_routing_enabled(conf):
        return _canonical_route_authoritative_with_aux_cuda(
            keys, pmid, disp, vel, acc, valid, global_nMesh, max_values_to_share, left_perm, right_perm, num_gpus,
    ...
    *send_left_items, send_left_pos = _compact_sorted_particles_with_slots(
    ...
    if num_gpus == 2:
        ...
        merged, stay_pos, stay_valid = _sparse_route_merge_two(
            original, send_left, send_left_pos, incoming_from_right, pmid.shape[0], key_fill,
            "[ERROR] Exceeded canonical authoritative capacity after migration. "
            "required_particles={x}, max_ptcl_per_slice={y}.", provenance=True, incoming_tag=jnp.int32(2),
        )
        send_right_pos = jnp.full((max_values_to_share, ), -1, dtype=jnp.int32)
        send_right_valid = jnp.zeros((max_values_to_share, ), dtype=jnp.bool_)
    else:
        *send_right_items, send_right_pos = _compact_sorted_particles_with_slots(
        ...
        merged, stay_pos, stay_valid = _sparse_route_merge_three(
            original, send_left, send_left_pos, send_right, send_right_pos, incoming_from_left, incoming_from_right,
            pmid.shape[0], key_fill, "[ERROR] Exceeded canonical authoritative capacity after migration. "
            "required_particles={x}, max_ptcl_per_slice={y}.", provenance=True,
        )
        send_right_valid = send_right[-1]

    route_aux = (
        stay_pos, stay_valid, send_left_pos, send_left[-1], send_right_pos, send_right_valid, merged[-2], merged[-1],
    )
    return merged[:6], route_aux


def _reverse_build_full_cot(
    full_cot, auth_pmid, auth_disp, auth_valid, halo_end, max_ptcl_per_slice, max_halo_values_to_share, global_nMesh,
    left_perm, right_perm, disp_size,
):
    ...
    right_halo_mask = auth_valid & particles_in_slice_mask(x_mod, halo_end.squeeze()[0], halo_end.squeeze()[1])
    right_halo_pos = jnp.compress(
        right_halo_mask, auth_pos, axis=0, size=max_halo_values_to_share, fill_value=jnp.asarray(-1, auth_pos.dtype),
    )
    ...
    left_cot = jnp.compress(
        left_mask, full_cot, axis=0, size=max_halo_values_to_share, fill_value=jnp.asarray(0, full_cot.dtype),
    )
    auth_cot = jnp.compress(
        auth_mask, full_cot, axis=0, size=auth_pmid.shape[0], fill_value=jnp.asarray(0, full_cot.dtype),
    )
    ...
    auth_cot = auth_cot.at[jnp.where(right_halo_valid, right_halo_pos,
                                     0)].add(outbound_right_cot * valid_mask.astype(full_cot.dtype))
    return auth_cot


def _reverse_route_cot(
    merged_cot, stay_pos, stay_valid, send_left_pos, send_left_valid, send_right_pos, send_right_valid, merge_src_tag,
    merge_src_idx, auth_size, max_values_to_share, left_perm, right_perm, conf=None,
):
    ...
    if conf is not None and cuda_routing_enabled(conf):
        stay_cot, incoming_from_left_cot, incoming_from_right_cot = (
            cuda_route_transpose_split(
                merged_cot, merge_src_tag, merge_src_idx, auth_size=auth_size, share_capacity=max_values_to_share,
            )
        )
        ...
        return cuda_route_transpose_scatter(
            stay_cot, send_left_cot, send_right_cot, stay_pos, stay_valid, send_left_pos, send_left_valid,
            send_right_pos, send_right_valid, auth_size=auth_size, share_capacity=max_values_to_share,
        )
    ...
    stay_cot = stay_cot.at[jnp.where(stay_mask, merge_src_idx, 0)].add(merged_cot * stay_scale)
    incoming_from_left_cot = incoming_from_left_cot.at[jnp.where(incoming_left_mask, merge_src_idx,
                                                                 0)].add(merged_cot * incoming_left_scale)
    incoming_from_right_cot = incoming_from_right_cot.at[jnp.where(incoming_right_mask, merge_src_idx,
                                                                   0)].add(merged_cot * incoming_right_scale)
    ...
    send_left_valid_scale = send_left_valid.reshape((send_left_valid.shape[0], ) + (1, ) *
                                                    (merged_cot.ndim - 1)).astype(dtype)
    send_right_valid_scale = send_right_valid.reshape((send_right_valid.shape[0], ) + (1, ) *
                                                      (merged_cot.ndim - 1)).astype(dtype)
    auth_cot = auth_cot.at[jnp.where(stay_valid, stay_pos, 0)].add(stay_cot * stay_valid_scale)
    auth_cot = auth_cot.at[jnp.where(send_left_valid, send_left_pos, 0)].add(send_left_cot * send_left_valid_scale)
    auth_cot = auth_cot.at[jnp.where(send_right_valid, send_right_pos, 0)].add(send_right_cot * send_right_valid_scale)
    return auth_cot


def _reverse_route_cot_two_gpu(
    merged_cot, stay_pos, stay_valid, send_left_pos, send_left_valid, _send_right_pos, _send_right_valid, merge_src_tag,
    merge_src_idx, auth_size, max_values_to_share, left_perm, right_perm, conf=None,
):
    ...
    if conf is not None and cuda_routing_enabled(conf):
        ...
        return cuda_route_transpose_scatter(
            stay_cot, send_left_cot, zero_send_right, stay_pos, stay_valid, send_left_pos, send_left_valid,
            _send_right_pos, _send_right_valid, auth_size=auth_size, share_capacity=max_values_to_share,
        )
...
def halo_move_pullback_from_prestate_shard_map(
    pmid, source_disp, carried_disp, vel, acc, halo_end, unused_indexes, disp_cot, vel_cot, acc_cot, global_nMesh,
    max_values_to_share, max_halo_values_to_share, max_ptcl_per_slice, left_perm, right_perm, num_gpus, disp_size,
    offsets, conf,
):
    ...
    (auth_keys, auth_pmid, auth_disp, auth_vel, auth_acc, auth_valid, auth_slots,
     ) = _canonical_authoritative_from_full_with_slots(
         pmid, source_disp, carried_disp, vel, acc, unused_indexes, global_nMesh, disp_size, num_gpus, offsets, conf,
     )
    (_merged_keys, merged_pmid, merged_disp, _merged_vel, _merged_acc, merged_valid,
     ), route_aux = _canonical_route_authoritative_with_aux(
         auth_keys, auth_pmid, auth_disp, auth_vel, auth_acc, auth_valid, global_nMesh, max_values_to_share, left_perm,
         right_perm, num_gpus, disp_size, offsets, conf,
     )
    (
        stay_pos, stay_valid, send_left_pos, send_left_valid, send_right_pos, send_right_valid, merge_src_tag,
        merge_src_idx,
    ) = route_aux

    payload_cot = jnp.stack((disp_cot, vel_cot, acc_cot), axis=-1)
    merged_payload_cot = _reverse_build_full_cot(
        payload_cot, merged_pmid, merged_disp, merged_valid, halo_end, max_ptcl_per_slice, max_halo_values_to_share,
        global_nMesh, left_perm, right_perm, disp_size,
    )

    _reverse_route_fn = _reverse_route_cot_two_gpu if num_gpus == 2 else _reverse_route_cot
    auth_payload_cot = _reverse_route_fn(
        merged_payload_cot, stay_pos, stay_valid, send_left_pos, send_left_valid, send_right_pos, send_right_valid,
        merge_src_tag, merge_src_idx, auth_pmid.shape[0], max_values_to_share, left_perm, right_perm, conf,
    )

    input_payload_cot = _scatter_compact_to_dense(auth_payload_cot, auth_slots, auth_valid, pmid.shape[0], )
...
def halo_move_pullback_mesh_halo_from_prestate_shard_map(
    ...
    (_merged_keys, _merged_pmid, _merged_disp, _merged_vel, _merged_acc, merged_valid,
     ), route_aux = _canonical_route_authoritative_with_aux(
         auth_keys, auth_pmid, auth_disp, auth_vel, auth_acc, auth_valid, global_nMesh, max_values_to_share, left_perm,
         right_perm, num_gpus, disp_size, offsets, conf,
     )
    ...
    payload_cot = jnp.stack((disp_cot, vel_cot, acc_cot), axis=-1)
    merged_payload_cot = _reverse_build_owned_only_cot(payload_cot, auth_pmid.shape[0], merged_valid, )
    _reverse_route_fn = _reverse_route_cot_two_gpu if num_gpus == 2 else _reverse_route_cot
    auth_payload_cot = _reverse_route_fn(
        merged_payload_cot, stay_pos, stay_valid, send_left_pos, send_left_valid, send_right_pos, send_right_valid,
        merge_src_tag, merge_src_idx, auth_pmid.shape[0], max_values_to_share, left_perm, right_perm, conf,
...
def _canonical_build_full_from_authoritative(
    auth_keys, auth_pmid, auth_disp, auth_vel, auth_acc, auth_valid, halo_start, halo_end, max_ptcl_per_slice,
    max_halo_values_to_share, global_nMesh, right_perm, disp_size, conf,
):
    ...
    outbound_right_halo = _compact_sorted_particles(
        auth_keys, auth_pmid, auth_disp, auth_vel, auth_acc, right_halo_mask, max_halo_values_to_share, key_fill,
    ...
    return _pack_left_halo_and_authoritative(
        *incoming_left_halo, auth_keys, auth_pmid, auth_disp, auth_vel, auth_acc, auth_valid, max_ptcl_per_slice,
        halo_start, halo_end, global_nMesh, disp_size,
    )


def move_particles_canonical_shard_map(
    pmid, disp_before, disp_after, vel, acc, halo_start, halo_end, unused_indexes, global_nMesh, max_values_to_share,
    max_halo_values_to_share, max_ptcl_per_slice, left_perm, right_perm, num_gpus, disp_size, offsets, conf,
):
    ...
    auth = _canonical_authoritative_from_full(
        pmid, disp_before, disp_after, vel, acc, unused_indexes, global_nMesh, disp_size, num_gpus, offsets, conf,
    )
...
def move_particles_mesh_halo_shard_map(
    ...
    auth = _authoritative_prefix_from_owned_only(pmid, disp_after, vel, acc, unused_indexes, conf, )
    ...
    pmid, disp, vel, acc, halo_mask, unused_indexes = _pack_authoritative_only(
        auth_pmid, auth_disp, auth_vel, auth_acc, auth_valid, max_ptcl_per_slice,
    )
    return pmid, disp, vel, acc, halo_mask, unused_indexes, jnp.bool_(False), max_particles_moved


def move_particles_mesh_halo_no_acc_shard_map(
    pmid, disp_before, disp_after, vel, halo_start, halo_end, unused_indexes, global_nMesh, max_values_to_share,
    max_halo_values_to_share, max_ptcl_per_slice, left_perm, right_perm, num_gpus, disp_size, offsets, conf,
):
...
def reconstruct_pre_drift_canonical_shard_map(
    pmid, disp, vel, acc, halo_start, halo_end, unused_indexes, drift_factor, global_nMesh, max_values_to_share,
    max_halo_values_to_share, max_ptcl_per_slice, left_perm, right_perm, num_gpus, disp_size, offsets, conf,
):
    ...
    auth_keys, auth_pmid, auth_disp, auth_vel, auth_acc, auth_valid = _canonical_authoritative_from_full(
        pmid, disp, disp, vel, acc, unused_indexes, global_nMesh, disp_size, num_gpus, offsets, conf,
    )
...
def reconstruct_pre_drift_mesh_halo_shard_map(
    ...
    auth_keys, auth_pmid, auth_disp, auth_vel, auth_acc, auth_valid = _authoritative_prefix_from_owned_only(
        pmid, disp, vel, acc, unused_indexes, conf,
    )
    auth_disp = auth_disp - auth_vel * drift_factor.astype(auth_disp.dtype)
...
def reconstruct_pre_drift_and_pullback_mesh_halo_shard_map(
    pmid, disp, vel, acc, unused_indexes, drift_factor, disp_cot, vel_cot, acc_cot, global_nMesh, max_values_to_share,
    max_halo_values_to_share, max_ptcl_per_slice, left_perm, right_perm, num_gpus, disp_size, offsets, conf,
):
...
def reconstruct_pre_drift_and_pullback_canonical_shard_map(
    ...
    disp_pullback, vel_pullback, acc_pullback = halo_move_pullback_from_prestate_shard_map(
        pre_pmid, pre_disp, disp_before_halo, pre_vel, pre_acc, halo_end, pre_unused_index, disp_cot, vel_cot, acc_cot,
        global_nMesh, max_values_to_share, max_halo_values_to_share, max_ptcl_per_slice, left_perm, right_perm,
        num_gpus, disp_size, offsets, conf,
    )
    return (
        pre_pmid, pre_disp, pre_vel, pre_acc, pre_unused_index, pre_halo_mask, disp_pullback, vel_pullback,
...
def _halo_capacity(conf):
    ...
    return min(
        ...
        (conf.max_ptcl_per_slice * conf.ptcl_halo_width + conf.local_mesh_shape[0] - 1) // conf.local_mesh_shape[0],
    )
...
def initialize_mGPU_halo_movement_canonical(conf):
    ...
    if conf.num_devices == 1:
        ...
        def _halo_noop(pmid, disp_before, disp_after, vel, acc, halo_start, halo_end, unused_indexes):
            ...
            return (
                pmid, disp_after, vel, acc, jnp.zeros_like(unused_indexes), unused_indexes, jnp.bool_(False),
                jnp.int32(0),
            )
    ...
    func = partial(
        move_fn, global_nMesh=conf.nMesh, max_values_to_share=conf.max_share_ptcl,
        max_halo_values_to_share=_halo_capacity(conf), max_ptcl_per_slice=conf.max_ptcl_per_slice,
        left_perm=conf.left_perm, right_perm=conf.right_perm, num_gpus=conf.num_devices, disp_size=conf.disp_size,
        offsets=conf.offsets, conf=conf,
    )
    return shard_map(
        func, mesh=conf.compute_mesh, in_specs=(
...
def initialize_mGPU_halo_movement_no_acc(conf):
    ...
    func = partial(
        move_particles_mesh_halo_no_acc_shard_map, global_nMesh=conf.nMesh, max_values_to_share=conf.max_share_ptcl,
        max_halo_values_to_share=_halo_capacity(conf), max_ptcl_per_slice=conf.max_ptcl_per_slice,
        left_perm=conf.left_perm, right_perm=conf.right_perm, num_gpus=conf.num_devices, disp_size=conf.disp_size,
        offsets=conf.offsets, conf=conf,
    )
...
def initialize_mGPU_reconstruct_pre_drift(conf):
    ...
    func = partial(
        reconstruct_fn, global_nMesh=conf.nMesh, max_values_to_share=conf.max_share_ptcl,
        max_halo_values_to_share=_halo_capacity(conf), max_ptcl_per_slice=conf.max_ptcl_per_slice,
        left_perm=conf.left_perm, right_perm=conf.right_perm, num_gpus=conf.num_devices, disp_size=conf.disp_size,
        offsets=conf.offsets, conf=conf,
    )
...
def initialize_mGPU_reconstruct_pre_drift_pullback(conf):
    ...
    func = partial(
        pullback_fn, global_nMesh=conf.nMesh, max_values_to_share=conf.max_share_ptcl,
        max_halo_values_to_share=_halo_capacity(conf), max_ptcl_per_slice=conf.max_ptcl_per_slice,
        left_perm=conf.left_perm, right_perm=conf.right_perm, num_gpus=conf.num_devices, disp_size=conf.disp_size,
        offsets=conf.offsets, conf=conf,
    )
    return shard_map(
        func, mesh=conf.compute_mesh, in_specs=(
            P(AXIS_NAME, None), P(AXIS_NAME, None), P(AXIS_NAME, None), P(AXIS_NAME, None), P(AXIS_NAME), P(),
...
def initialize_mGPU_halo_move_pullback(conf):
    ...
    if conf.num_devices == 1:
        ...
        def _pullback_noop(
            pmid, source_disp, carried_disp, vel, acc, halo_end, unused_indexes, disp_cot, vel_cot, acc_cot,
        ):
...
def initialize_mGPU_compute_halo_mask(conf):
    ...
    func = partial(compute_halo_mask_shard_map, global_nMesh=conf.nMesh, disp_size=conf.disp_size, )
    return shard_map(
