...
The core :mod:`pmpp` wheel deliberately does not contain a precompiled CUDA
extension.  This module is therefore written so importing PM++ never loads
``libcuda`` or ``nvcc``-built code.  When a locally built shared library is
present, the shard-local handlers exposed here replace only the local pack and
merge parts of ``mesh_halo`` routing.  Neighbor communication remains ordinary
JAX ``lax.ppermute`` operations.
...
from ._cuda_paths import user_cache_cuda_directory
...
def _candidate_library_paths() -> tuple[Path, ...]:
    ...
    paths.extend((
        package_dir / "_cuda" / "libpmpp_cuda_routing.so", package_dir / "_cuda" / "pmpp_cuda_routing.so",
        user_cache_cuda_directory() / "libpmpp_cuda_routing.so", user_cache_cuda_directory() / "pmpp_cuda_routing.so",
...
def _register_targets(*, strict: bool = False) -> bool:
    ...
    if not _FLOAT64_REGISTERED and all(hasattr(library, target) for target in _FLOAT64_TARGETS):
        try:
    ...
    if not _FLOAT64_BIDIR_REGISTERED and all(hasattr(library, target) for target in _FLOAT64_BIDIR_TARGETS):
        try:
...
def extension_status() -> dict[str, Any]:
    ...
    return {
        ...
        "library":
        None if library is None else getattr(library, "_name", None),
        "registered":
        bool(_REGISTERED),
        "bidir_registered":
        bool(_BIDIR_REGISTERED),
        "float64_registered":
        ...
        manifest,
        "candidates":
        tuple(str(path) for path in _candidate_library_paths()),
    }


def supported_configuration(conf: Any, *, num_devices: int | None = None, mode: str | None = None) -> bool:
    ...
    if jnp.dtype(conf.pmid_dtype) not in {jnp.dtype(jnp.int16), jnp.dtype(jnp.int32)}:
        return False
    resolved_num_devices = (int(num_devices) if num_devices is not None else int(getattr(conf, "num_devices", 0) or 0))
...
def supported_bidir_configuration(conf: Any, *, num_devices: int | None = None, mode: str | None = None) -> bool:
...
def route_pack(
    pmid: jax.Array, disp: jax.Array, vel: jax.Array, valid: jax.Array, x_mod: jax.Array, *, global_nmesh: int,
    mesh_shape: tuple[int, int, int], owned_start: int, owned_end: int, slice_width: int, direction: int,
    ...
    outputs = (
        _shape_dtype((capacity, record_words), jnp.uint32), _shape_dtype((), jnp.int32), _shape_dtype((n, ), jnp.uint8),
    )
    ...
    return call(
        ...
        jnp.asarray(slice_width, dtype=jnp.int32), global_nmesh=np.int32(global_nmesh), mesh_x=np.int32(mesh_shape[0]),
        mesh_y=np.int32(mesh_shape[1]), mesh_z=np.int32(mesh_shape[2]), direction=np.int32(direction),
        num_devices=np.int32(num_devices), capacity=np.int32(capacity),
    )


def route_pack_bidir_cuda(
    pmid: jax.Array, disp: jax.Array, vel: jax.Array, valid: jax.Array, x_mod: jax.Array, *, global_nmesh: int,
    mesh_shape: tuple[int, int, int], owned_start: int, owned_end: int, slice_width: int, num_devices: int,
    capacity: int, stay_capacity: int | None = None,
) -> tuple[jax.Array, ...]:
    ...
    return jax.ffi.ffi_call(f"pmpp_route_bidir_pack{target_suffix}", outputs)(
        ...
        capacity=np.int32(capacity), stay_capacity=np.int32(stay_capacity),
    )
...
def route_merge(
    pmid: jax.Array, disp: jax.Array, vel: jax.Array, stay_mask: jax.Array, incoming_records: jax.Array,
    incoming_count: jax.Array, *, mesh_shape: tuple[int, int, int], capacity: int, auxiliary: bool = False,
) -> tuple[jax.Array, ...]:
    ...
    outputs: tuple[jax.ShapeDtypeStruct, ...] = (
        _shape_dtype((capacity, 3), jnp.int32), _shape_dtype((capacity, 3), float_dtype),
        _shape_dtype((capacity, 3), float_dtype), _shape_dtype((capacity, ), jnp.uint8),
    )
    if auxiliary:
        outputs += (_shape_dtype((capacity, ), jnp.uint8), _shape_dtype((capacity, ), jnp.int32), )
        target = f"pmpp_route_merge_aux{target_suffix}"
    else:
        target = f"pmpp_route_merge{target_suffix}"
    call = jax.ffi.ffi_call(target, outputs)
    return call(
        pmid.astype(jnp.int32), disp.astype(float_dtype), vel.astype(float_dtype), stay_mask.astype(jnp.uint8),
        incoming_records.astype(jnp.uint32), incoming_count.astype(jnp.int32), mesh_x=np.int32(mesh_shape[0]),
        mesh_y=np.int32(mesh_shape[1]), mesh_z=np.int32(mesh_shape[2]), capacity=np.int32(capacity),
    )


def route_transpose_split(
    merged_cot: jax.Array, source_tag: jax.Array, source_idx: jax.Array, *, auth_size: int, share_capacity: int,
) -> tuple[jax.Array, jax.Array, jax.Array]:
    ...
    outputs = (
        _shape_dtype((auth_size, ) + payload_shape,
                     float_dtype), _shape_dtype((share_capacity, ) + payload_shape, float_dtype),
    ...
    return jax.ffi.ffi_call(f"pmpp_route_transpose_split{target_suffix}", outputs)(
        merged_cot.astype(float_dtype), source_tag.astype(jnp.uint8), source_idx.astype(jnp.int32),
        auth_size=np.int32(auth_size), share_capacity=np.int32(share_capacity),
    )


def route_transpose_scatter(
    stay_cot: jax.Array, send_left_cot: jax.Array, send_right_cot: jax.Array, stay_pos: jax.Array,
    stay_valid: jax.Array, send_left_pos: jax.Array, send_left_valid: jax.Array, send_right_pos: jax.Array,
    send_right_valid: jax.Array, *, auth_size: int, share_capacity: int,
) -> jax.Array:
    """Scatter returned route cotangents into authoritative source slots."""
    float_dtype, _, target_suffix = _float_abi(stay_cot, send_left_cot, send_right_cot)
    ...
    return jax.ffi.ffi_call(f"pmpp_route_transpose_scatter{target_suffix}", output)(
        stay_cot.astype(float_dtype), send_left_cot.astype(float_dtype), send_right_cot.astype(float_dtype),
        stay_pos.astype(jnp.int32), stay_valid.astype(jnp.uint8), send_left_pos.astype(jnp.int32),
        send_left_valid.astype(jnp.uint8), send_right_pos.astype(jnp.int32), send_right_valid.astype(jnp.uint8),
        auth_size=np.int32(auth_size), share_capacity=np.int32(share_capacity),
    )
