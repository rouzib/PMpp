...
def _mesh_conditioning_channels(source, potential_real, a, cosmo, dtype):
    ...
    cosmo_channels = [jnp.broadcast_to(jnp.asarray(value, dtype=dtype), spatial_shape) for value in features]
...
def _mesh_conditioning_vector(a, cosmo, dtype):
    ...
    return jnp.concatenate([jnp.asarray([a], dtype=dtype), cosmo_features(cosmo, dtype), ])
...
def _periodic_pad_mesh_channels(x, halo_width, conf):
    ...
    if halo_width <= 0:
        ...
        incoming_left, incoming_right = exchange_owned_mesh_halo_edges(x, halo_width, conf.left_perm, conf.right_perm, )
...
def _periodic_conv3d(x, output_channels, conf, kernel_shape=3, name=None, w_init=None, b_init=None):
    ...
    conv = hk.Conv3D(
        output_channels=output_channels, kernel_shape=kernel_shape, padding="VALID", data_format="NDHWC", w_init=w_init,
        b_init=b_init, name=name,
    )
...
class MeshResidualPotentialCorrection(HaikuModuleBase):
    ...
    def __call__(self, source, potential_real, a, cosmo, conf):
        dtype = source.dtype
        source_scale = jnp.sqrt(_global_mesh_mean(source**2, conf) + jnp.asarray(1e-6, dtype=dtype))
        source_norm = source / source_scale
        potential_scale = jnp.sqrt(_global_mesh_mean(potential_real**2, conf) + jnp.asarray(1e-6, dtype=dtype))
        ...
        for block_id in range(max(self.depth - 1, 0)):
            residual = x
            x = jax.nn.gelu(_periodic_conv3d(x, self.channels, conf, kernel_shape=3, name=f"block_{block_id}_conv1"))
        ...
        out = _periodic_conv3d(
            x, 1, conf, kernel_shape=3, name="out", w_init=hk.initializers.Constant(0.0),
            b_init=hk.initializers.Constant(0.0),
        )
        conv_residual = self.max_residual * jnp.tanh(out[..., 0]) * potential_scale
        direct_gain = self.max_residual * jnp.tanh(
            hk.Linear(
                1, name="direct_gain", w_init=hk.initializers.RandomNormal(stddev=self.output_init_scale),
                b_init=hk.initializers.Constant(0.0),
            )(conditioning)[0]
        )
    ...
    return hk.without_apply_rng(
        hk.transform(
            lambda source, potential_real, a, c: MeshResidualPotentialCorrection(
                channels=channels, depth=depth, max_residual=max_residual, output_init_scale=output_init_scale,
...
@partial(
    pytree_dataclass, aux_fields=(
        "channels", "depth", "max_residual", "output_init_scale", "allow_missing_sigma8", "sigma8_value", "dtype"
...
def init_mesh_cnn_potential_correction(
    key, channels=8, depth=4, max_residual=0.1, output_init_scale=1e-2, allow_missing_sigma8=False, sigma8_value=0.8,
    ...
    params = transform.init(key, source, potential_real, jnp.asarray(1.0, dtype=dtype), default_cosmo_features(dtype), )
    return MeshCNNPotentialCorrection(
        params=params, channels=channels, depth=depth, max_residual=max_residual, output_init_scale=output_init_scale,
...
def _apply_mesh_cnn_residual(params, source_real, potential_real, a, cosmo_features_in, conf, correction):
    ...
    transform = mesh_cnn_transform(
        conf, correction.channels, correction.depth, correction.max_residual, correction.output_init_scale,
    )
    residual = transform.apply(params, source_real, potential_real, a, cosmo_features_in, )
...
def evaluate_mesh_potential_residual(correction, source_real, potential_real, a, cosmo, conf):
    ...
    return apply_fn(
        ...
        jnp.asarray(a, dtype=correction.dtype), correction_cosmo_features(correction, cosmo, correction.dtype),
    )
