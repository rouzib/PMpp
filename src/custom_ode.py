...
def odeint(func, y0, t, *args, rtol=1.4e-8, atol=1.4e-8, mxstep=jnp.inf, hmax=jnp.inf, custom_func=None):
    ...
    Args:
      ...
      *args: tuple of additional arguments for `func`, which must be arrays
      ...
      custom_func: Callable, function that takes arguments `y[t]` and `t` and that will be called for each t with the
        corresponding y.
    ...
    return _odeint_wrapper(converted, rtol, atol, mxstep, hmax, y0, t, custom_func, *args, *consts)


@partial(jax.jit, static_argnums=(0, 1, 2, 3, 4, 7))
def _odeint_wrapper(func, rtol, atol, mxstep, hmax, y0, ts, custom_func, *args):
    y0, unravel = ravel_pytree(y0)
    func = ravel_first_arg(func, unravel)
    out = _odeint(func, rtol, atol, mxstep, hmax, y0, ts, custom_func, *args)
    return jax.vmap(unravel)(out)


@partial(jax.custom_vjp, nondiff_argnums=(0, 1, 2, 3, 4, 7))
def _odeint(func, rtol, atol, mxstep, hmax, y0, ts, custom_func, *args):
    ...
    def scan_fun(carry, target_t):
        ...
        if custom_func is None:
            ...
            custom_func(y_target.reshape(2, -1, 3), target_t)
            # returns target_t instead of y_target for memory management and so that the function is callable with lax.scan
            return carry, target_t
    ...
    if custom_func is None:
        ...
        lax.scan(scan_fun, init_carry, ts[1:])
        return y0[None]


def _odeint_fwd(func, rtol, atol, mxstep, hmax, y0, ts, custom_func, *args):
    ys = _odeint(func, rtol, atol, mxstep, hmax, y0, ts, custom_func, *args)
    return ys, (ys, ts, args)


def _odeint_rev(func, rtol, atol, mxstep, hmax, res, g, custom_func):
    ys, ts, args = res
